...
public abstract class GameThread extends Thread {
	...
	int levelPlayed = 1;
	int totalScore = 0;
	boolean customLevel = false; //flag for user created level
	protected int custScore, custObstacle, custEnemyB, custEnemyP;
	...
	public boolean onTouch(MotionEvent e) {
		...
		if(mMode == STATE_READY || mMode == STATE_LOSE || (mMode == STATE_WIN && levelPlayed <4)) {
			doStart();
			return true;
		}
				...
				if (mMode == STATE_READY)
					...
					if (mMode == STATE_PAUSE)
						...
						if (mMode == STATE_LOSE)
							...
							if (mMode == STATE_WIN && levelPlayed ==4) {
